...
class Step3():
	...
	def crowded_init_new(self, state: list):
		if state.count(1) > self.limit:
			crowded = 1
		else:
			crowded = 0
		return crowded
	...
	def generation_work(self, strategyset: list):
		...
		strategy_best_one, strategy_best_two, eva_store, strategy = self.evaluate(strategyset, eva)
		...
		# eva_store.pop()
		return strategy_best_one, strategy_best_two, strategy, eva_store
	...
	def evaluate(self, strategyset: list, eva: list):  # this function is used to sort the strategy list
		...
		strategy_best_two = strategy.pop()
		eva_store.pop()
		return strategy_best_one, strategy_best_two, eva_store, strategy

	def evaluate2(self, strategyset, eva):
		eva_overall = list(zip(strategyset, eva))
		eva_cal = sorted(eva_overall, key=lambda x: x[1], reverse=True)
		strategy, eva_store = map(list, zip(*eva_cal))
		strategy_best_one = strategy.pop()
	...
	def evacalculate2(self, state: list, crowded: int, data: list):
		eva_out = 0
		for i in range(self.weeks):
			state, crowded, eva = self.weekwork2(data=data, crowded=crowded, state=state)
			eva_out += eva
			print(i, self.generation, state.count(1), crowded, *state)
		return eva_out

	def weekwork2(self, data, crowded, state):
		new_state = []
		for j in range(self.population_size):
			prob = data[crowded][state[j]][1]
			prob2 = random.random()
			if 0 <= prob2 < prob:
				new_state.append(1)
			else:
	...
	def generation_work2(self, strategyset: list):
		probset, Data = self.parsego(strategySet=strategyset)
		state_set = self.state_init_go(probset=probset)
		crowded_set = self.crowded_init(state_set=state_set)
		eva = [0] * len(strategyset)
		for i in range(self.weeks):
			state_temp, crowded_temp, eva_temp = self.weekwork(data=Data, crowded=crowded_set, state=state_set)
			for j in range(len(state_temp)):
				print(i, '\t', self.generation, '\t', state_temp[j].count(1), '\t', crowded_temp[j], '\t',
				      *state_temp[j])
			eva = list(map(lambda x, y: x + y, eva, eva_temp))
			state_set = state_temp
			crowded_set = crowded_temp
		strategy_best_one, eva_store, strategy = self.evaluate2(strategyset, eva)
		# eva_overall = list(zip(strategyset,eva))
		# eva_cal = sorted(eva_overall,key=lambda x:x[1],reverse=True)
		# strategy,eva_store = map(list,zip(*eva_cal))
		# strategy_best_one = strategy.pop()
	...
	def newalgorithm(self, population_size: int, states: int):
		t1 = time.clock()
		strategyset = self.create_strategy(population_size, states)
		strategy_one, strategyset, eva_store = self.generation_work2(strategyset)
		Condition = True
		while Condition:
			strategy_one = self.Mutation(In=strategy_one)
			prob, data = self.parser(strategy_one)
			state_search = self.state_init(prob, population=self.population_size)
			crowded_search = self.crowded_init_new(state=state_search)
			eva_temp = self.evacalculate2(state=state_search, crowded=crowded_search, data=data)
			strategyset.append(strategy_one)
			eva_store.append(eva_temp)
			strategy_one, eva_store, strategyset = self.evaluate2(strategyset=strategyset, eva=eva_store)
			self.generation = self.generation + 1
			t2 = time.clock()
			Condition = t2 - t1 < self.max_t
		return strategy_one
...
# print(evacal)
best = s3.newalgorithm(population_size=5, states=2)
print(best)
